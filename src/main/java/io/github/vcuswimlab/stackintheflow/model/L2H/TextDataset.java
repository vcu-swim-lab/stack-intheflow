...
public class TextDataset extends AbstractTokenizeDataset {
    ...
    public void loadFormattedData(String fFolder) {
        ...
        loadFormattedData(
				getClass().getResourceAsStream("/" + Paths.get(fFolder, formatFilename + wordVocabExt).toString()),
				getClass().getResourceAsStream("/" + Paths.get(fFolder, formatFilename + numDocDataExt).toString()),
				getClass().getResourceAsStream("/" + Paths.get(fFolder, formatFilename + docInfoExt).toString()),
				getClass().getResourceAsStream("/" + Paths.get(fFolder, formatFilename + numSentDataExt).toString()));
...
//                new File(fFolder, formatFilename + numSentDataExt));
	...
	public void loadFormattedData(InputStream wordVocabIs,
            InputStream docWordIs,
            InputStream docInfoIs,
            InputStream sentIs) {

        try {
            inputWordVocab(wordVocabIs);
            inputTextData(docWordIs);
            if (docInfoIs != null) {
                inputDocumentInfo(docInfoIs);
            }
            if (sentIs != null) {
                inputSentenceTextData(sentIs);
            }
        } catch (Exception e) {
    ...
    protected void inputWordVocab(File file) throws Exception {
		inputWordVocab(new FileInputStream(file));
	}

	protected void inputWordVocab(InputStream is) throws Exception {
    ...
    protected void inputTextData(File file) throws Exception {
        ...
        inputTextData(new FileInputStream(file));
	}

	protected void inputTextData(InputStream is) throws Exception {
        words = inputFormattedTextData(is);
	...
	protected int[][] inputFormattedTextData(File file) throws Exception {
        ...
        return inputFormattedTextData(new FileInputStream(file));
    }

	protected int[][] inputFormattedTextData(InputStream is) throws Exception {
        BufferedReader reader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
    ...
    protected void inputSentenceTextData(File file) throws Exception {
        ...
        inputSentenceTextData(new FileInputStream(file));
     }

	protected void inputSentenceTextData(InputStream is) throws Exception {
        BufferedReader numSentReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
...
//        }
    ...
    protected void inputDocumentInfo(File file) throws Exception {
        ...
        inputDocumentInfo(new FileInputStream(file));
    }

	protected void inputDocumentInfo(InputStream is) throws Exception {
        BufferedReader reader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
        String line;
